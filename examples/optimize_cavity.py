...
def fitness(cavity_params):
    ...
    cavity = sivp.OvercoupledPCCv0p4p2(pcc_params_7_3)
    ...
    engine = LumericalEngine(mesh_accuracy=2, hide=True, lumerical_path=FDTDLoc, working_path="./fsps",save_fsp=False)
    ...
    for hx, hy, a in zip(all_hx, all_hy, all_a):
        ...
        cell_hole = CylinderStructure(Vec3(0), beam_h, hx/2, DielectricMaterial(1, order=1, color="red"), radius2=hy/2)
    ...
    cavity = Cavity1D(
      ...
      structures=[TriStructure(Vec3(0), Vec3(beam_w, apex_half_angle, beam_length), 
                  DielectricMaterial(2.4028, order=2, color="gray"), rot_angles=(np.pi/2, np.pi/2, 0))],
    ...
    r1 = cavity.simulate("resonance", target_freq=target_frequency, source_pulselength=200e-15, 
                        analyze_time=1000e-15, mesh_regions = [], sim_size=Vec3(2, 4, 8))
...
log_name = "log_test_optimal-tri_090722_00.txt"
#p0 = np.array([0.270,0.250,0.114,0.161,0.1392,0.482])
p0 = np.array([0.270,0.260,0.114,0.161,0.1392,0.482])
